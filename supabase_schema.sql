-- ============================================================================
-- RAGHAV TEXCHEMS CHEMICAL PRIVATE LIMITED - ENTERPRISE DATABASE SCHEMA
-- ============================================================================
-- Clean Re-creation: Drops previous tables and functions cascade, then creates
-- the production-hardened PostgreSQL schema with Row-Level Security (RLS),
-- indexes, triggers, storage buckets, and realtime synchronization.
-- ============================================================================
...
-- ============================================================================
-- 0. CLEAN DROP OF PREVIOUS OBJECTS (CASCADING)
-- ============================================================================
...
-- ============================================================================
-- 2. ADMIN USERS TABLE & AUTHORIZATION (OWASP A01: Broken Access Control)
-- ============================================================================
...
-- Helper function: Check if current authenticated user is an active Admin
...
-- ============================================================================
-- 11. BASE SINGLETON INITIALIZATION
-- ============================================================================
...
VALUES (
  'primary',
  'https://strvid.nyc3.cdn.digitaloceanspaces.com/motionsite/dna_video.mp4',
  'direct',
  'Our Story — Chemistry That Connects',
  '["Founded with a bold commitment to precision chemical formulation, Raghav Texchems Chemical Private Limited has established itself as an innovative manufacturer and exporter of Dyestuffs, Polymer Emulsions, and Specialty Auxiliaries.", "Operating with certified quality controls, modern testing laboratories, and responsible environmental compliance, we serve global and domestic industrial clients with unmatched batch consistency."]'::jsonb,
  'Our Mission',
  'To engineer sustainable, high-performance chemical formulations that empower global textile, paper, and polymer industries while upholding the highest environmental standards.',
  'Our Vision',
  'To be the most trusted specialty chemical manufacturing partner across Asia and international markets, driving progress through chemistry that connects.',
  '[]'::jsonb,
  '[]'::jsonb
)
ON CONFLICT (id) DO NOTHING;
...
BEGIN
  ...
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.inquiries; EXCEPTION WHEN others THEN NULL; END;
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.company_settings; EXCEPTION WHEN others THEN NULL; END;
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.stats; EXCEPTION WHEN others THEN NULL; END;
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.collaborations; EXCEPTION WHEN others THEN NULL; END;
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.about_content; EXCEPTION WHEN others THEN NULL; END;
END $$;
